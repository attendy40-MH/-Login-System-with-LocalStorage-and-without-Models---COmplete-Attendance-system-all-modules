...
const Teacher = () => {
    ...
    // Initialize system data and check authentication
    useEffect(() => {
    ...
    const autoLoginTeacher = () => {
        // Auto-login teacher1 for testing via backend
        (async () => {
            const r = await system.login('teacher1', 'teacher123');
            if (r && r.success) {
                setCurrentUser(r.user);
            } else {
                // fallback: set a minimal local user for UI continuity
                const teacherUser = { username: 'teacher1', role: 'teacher', name: 'Ali Ahmed', courses: ['CS101'] };
    ...
    const generateQRCode = async () => {
        ...
        if (result && result.ok) {
            ...
            localStorage.setItem('currentQR', result.qrString);
